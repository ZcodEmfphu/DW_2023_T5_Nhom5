...
public class LoadFromDataWareHousetoDataMart {
  ...
  public void LoadDatawarehouseToDataMart() {
    try {
      ...
      String dwQuery =
          "SELECT dd.full_date AS Date_ef, MAX(CURTIME()) AS Time, '2999-12-31' AS Date_ex, pf.Product_Id, pd.Name AS Product_Name, pf.BuyingPrice, pf.SellingPrice "
      ...
      try (PreparedStatement dwStatement = dwConnection.prepareStatement(dwQuery);
        ...
        try {
          ...
          String dmQuery =
              "INSERT INTO AGGREGATE (Date_ef, Time, Date_ex, Product_Id, Product_Name, BuyingPrice, SellingPrice) "
                  + "VALUES (?, ?, ?, ?, ?, ?, ?)";
